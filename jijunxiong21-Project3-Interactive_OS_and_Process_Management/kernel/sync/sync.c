...
int do_barrier_init(int key, int goal)
{
    ...
    barr->pid = current_running[core_id]->pid;
...
void do_barrier_wait(int bar_idx)
{
    ...
    else
    {
        ...
        int core_id = get_current_cpu_id();
        do_block(&(current_running[core_id]->list), &(barr->block_queue), &(barr->lock)); /* inside critical section change */
    }
...
int do_semaphore_init(int key, int init)
{
    ...
    sema->key = key;
    int core_id = get_current_cpu_id();
    sema->pid = current_running[core_id]->pid;
...
void do_semaphore_down(int sema_idx)
{
    ...
    if (sema->value > 0)
        ...
        do_block(&(current_running[core_id]->list), &(sema->block_queue), &(sema->lock)); /* inside critical section change */
...
int do_mbox_send(int mbox_idx, void * msg, int msg_length)
{
    ...
    while (mbox_avail_len = (mbox->begin <= mbox->end
        ...
        // msg_length -= mbox_avail_len;
        int core_id = get_current_cpu_id();
        do_block(&(current_running[core_id]->list), &(mbox->send_queue),
...
int do_mbox_recv(int mbox_idx, void * msg, int msg_length)
{
    ...
    while (mbox_avail_len = (mbox->begin <= mbox->end
        ...
        // msg_length -= mbox_avail_len;
        int core_id = get_current_cpu_id();
        do_block(&(current_running[core_id]->list), &(mbox->recv_queue), &(mbox->lock)); /* inside critical section change */
    }
