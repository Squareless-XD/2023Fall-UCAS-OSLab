...
static void screen_write_ch(char ch)
{
    ...
    if (ch == '\n')
    {
        current_running[core_id]->cursor_x = 0; // move cursor to the beginning
        current_running[core_id]->cursor_y++;   // move cursor to the next line
    }
    else if (ch == '\r')
    {
        current_running[core_id]->cursor_x = 0; // move cursor to the beginning
        // current_running[core_id]->cursor_y++;   // do not move cursor to the next line
    }
    else if (ch == '\b')
    {
        printv("%c%c1X", CSI_0, CSI_1);
        if (current_running[core_id]->cursor_x > 0)
            current_running[core_id]->cursor_x--; // move cursor to the beginning
        new_screen[SCREEN_LOC(current_running[core_id]->cursor_x, current_running[core_id]->cursor_y)] = ' '; // write empty space to screen buffer
    }
    else
    {
        new_screen[SCREEN_LOC(current_running[core_id]->cursor_x, current_running[core_id]->cursor_y)] = ch; // write char to screen buffer
        current_running[core_id]->cursor_x++; // move cursor to the next column
    }
...
void screen_reflush(void)
{
    ...
    int core_id = get_current_cpu_id();
    vt100_move_cursor(current_running[core_id]->cursor_x + 1, current_running[core_id]->cursor_y + 1);
