...
void do_barrier_wait(int bar_idx)
{
    ...
    else
    {
        ...
        do_block(&(my_cpu()->list), &(barr->block_queue), &(barr->lock)); /* inside critical section change */
    }
...
void do_semaphore_down(int sema_idx)
{
    ...
    if (sema->value > 0)
        ...
        do_block(&(my_cpu()->list), &(sema->block_queue), &(sema->lock)); /* inside critical section change */
...
int do_mbox_send(int mbox_idx, void * msg, int msg_length)
{
    ...
    while (mbox_avail_len = (mbox->begin <= mbox->end
        ...
        // msg_length -= mbox_avail_len;
        do_block(&(my_cpu()->list), &(mbox->send_queue),
...
int do_mbox_recv(int mbox_idx, void * msg, int msg_length)
{
    ...
    while (mbox_avail_len = (mbox->begin <= mbox->end
        ...
        // msg_length -= mbox_avail_len;
        do_block(&(my_cpu()->list), &(mbox->recv_queue), &(mbox->lock)); /* inside critical section change */
    }
