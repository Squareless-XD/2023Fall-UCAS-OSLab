...
#define PAGE_SIZE 0x1000 // sector size
#define BOOT_LOADER_SIG_OFFSET 0x1fc // boot loader signature offset
#define OS_SIZE_LOC 0x1f8 // os size location, storing os size by 2 bytes
...
#define TASK_MAXNUM 32 // max number of tasks
...
static void create_image(int nfiles, char *files[])
{
        ...
        for (int ph = 0; ph < ehdr.e_phnum; ph++) {
            ...
            if (ph != 0) {
                write_padding(img, &phyaddr, phyaddr - phdr_last.p_filesz
                              + (phdr.p_vaddr - phdr_last.p_vaddr));
            }
            phdr_last = phdr;
            ...
            else if (taskidx >= 0)
            {
                ...
                taskinfo[taskidx].memsz += phdr.p_memsz; // calculate task memory size
            }
        }
    ...
    // padding to the end of the last sector, to get align with a page (0x1000) of CPU
    write_padding(img, &phyaddr, ROUND(phyaddr, PAGE_SIZE));

    // check tasks info
    for (int i = 0; i < tasknum; i++)
    {
        printf("%lx %x %x %x %s\n", taskinfo[i].entry, taskinfo[i].phyaddr, taskinfo[i].filesz, taskinfo[i].memsz, taskinfo[i].task_name);
    }
                           ...
                           int tasknum, FILE * img)
{
    ...
    int kernel_sec_num = NBYTES2SEC(nbytes_kernel);
    // calculate app volume size
    int task_off = SECTOR_SIZE + nbytes_kernel;
    ...
    fwrite(&kernel_sec_num, sizeof(int), 1, img); // write os size to image file

    // write app volume offset (to BOOT_LOADER_SIG_OFFSET)
    fseek(img, BOOT_LOADER_SIG_OFFSET, SEEK_SET); // locate to boot loader signature offset, from the beginning of the file, offset is BOOT_LOADER_SIG_OFFSET
    fwrite(&task_off, sizeof(int), 1, img); // write app volume size to image file
